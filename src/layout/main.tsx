			...
			<div className="h-20 bg-#201E18FF text-white flex items-center justify-between px-10">
				...
				<div className='flex'>
					{navList.map((item,index)=>{
						return (
							<div onClick={()=>navigate(item.path)} className="mx-10 cursor-pointer" key={index}>{item.title}</div>
						)
					})}
				</div>
				<div className='flex'>
					<div>{isDark?'dark':'light'}</div>
					<div className='cursor-pointer' onClick={()=>changeTheme(!isDark)}>改变模式</div>
				</div>
			</div>
			<div>
				<Outlet />
